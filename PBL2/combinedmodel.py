#!/usr/bin/env python3
"""
Synthetic cell + stirred bioreactor model for heparin production.

**Primary module** — run with: ``python combinedmodel.py``
(``synthetic_cell_heparin_bioreactor`` re-exports this file for compatibility.)

This script merges the project’s separate pieces into one runnable model:

  1. Transcription / translation (lumped E. coli–style mRNA and protein; from
     `simple transcription and translation model.txt`).
  2. Membrane diffusion of nutrients between extracellular bioreactor fluid and
     the cytosol (Fick’s law; from `diffusion.ipynb`). Glucose exchange is
     coupled to the metabolic ODE.
  3. Detailed glucose → heparosan → heparin pathway with cofactor recycling
     (`glucose_to_heparin.py`).
  4. Optional lumped sulfation ODE (standalone reference; same chemistry as the
     early `sulfation.py` prototype).
  5. Exocytosis: Hill packaging of cytosolic “mature” heparin (modeled as H3S
     from the detailed pathway), Ca²⁺-triggered release, vesicle pools, secreted
     product (`pbl2_heparin_exocytosis.ipynb`).

Time base: minutes (consistent with the exocytosis notebook). Diffusion was
originally per second; influx terms are multiplied by 60 when used here.

Troubleshooting unphysical behaviour:

  * **Bulk glucose crashes to ~0** — With ``NUMBER_OF_CELLS`` ≈ 1e10 and a 1 L
    reactor, hexokinase removes glucose faster than diffusion can refill unless
    you add a **fed-batch / chemostat** term (see ``feed`` in
    ``combined_synthetic_cell_ode``) or reduce cell number / uptake.
  * **Protein explodes to 1e4–1e5** — Expected for the linear TX/TL parameters
    (steady-state protein ≈ ``k_R k_P / (γ_R γ_P)``). Use normalized scaling
    (``scale`` uses ``P / P_ss``) so pathway modulation stays O(1); plot mRNA and
    protein on **two y-axes** so both curves are visible.

State layout for `combined_synthetic_cell_ode` (42 states):

  Pathway (35): same order as `heparin_pathway_ode` in `glucose_to_heparin.py`
  Exocytosis (4): R, C, V, S — reserve vesicles, cytosolic Ca, release vesicles,
                  secreted heparin (bulk reactor concentration proxy, same units
                  as pathway concentrations unless you rescale).
  Bioreactor glucose (1): G_out — extracellular glucose (mol/m³); intracellular
                  glucose is y[8] and is treated as mol/m³ numerically (= mM).
  Gene expression (2): m, P — mRNA and protein (arbitrary units).
...
# ---------------------------------------------------------------------------
# 0. Shared kinetic helpers (from glucose_to_heparin.py)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# 1. Lumped sulfation (reference only; from sulfation.py)
#    Heparosan + PAPS -> Heparin + PAP  with  v = k * Hs * PAPS
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# 2. Membrane diffusion (from diffusion.ipynb)
# ---------------------------------------------------------------------------
...
def steady_state_protein(gene: dict) -> float:
    """Protein level at dm/dt = dP/dt = 0 for constant transcription/translation."""
    return (gene["k_R"] / gene["gamma_R"]) * (gene["k_P"] / gene["gamma_P"])


def default_feed():
    """Fed-batch style glucose replenish on ``G_out`` (set ``enabled`` False for batch-only)."""
    return {"enabled": True, "k_feed": 0.12, "G_feed": 5.0}


def default_bioreactor_geometry():
    """Spherical cell + well-mixed bulk; same spirit as diffusion.ipynb."""
    cell_radius = 10e-6  # m
...
# ---------------------------------------------------------------------------
# 3. Exocytosis helpers (from pbl2_heparin_exocytosis.ipynb)
# ---------------------------------------------------------------------------
...
def f_hep(t, y, c, k_sulfation, pulse_times):
    """
    Standalone lumped sulfation + exocytosis (from pbl2_heparin_exocytosis.ipynb).
...
# ---------------------------------------------------------------------------
# 4. Core pathway ODE (from glucose_to_heparin.py) — returns derivatives list
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# 5. Gene expression (continuous-time version of the Euler model in the .txt)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# 6. Combined synthetic cell + bioreactor
# ---------------------------------------------------------------------------
...
def combined_synthetic_cell_ode(t, y, p, pulse_times, geo, exo, gene, scale, feed=None):
    """
    Coupling rules:
      ...
      * Optional enzyme_scale(P) multiplies selected Vmax values. **P is normalized
        by the TX/TL steady-state protein** so fold-change stays O(1) (raw P is
        often 1e4–1e5 with default gene parameters).
      * Optional ``feed``: first-order replenishment of bulk glucose
        ``dG_out += k_feed * (G_feed - G_out)`` (1/min) to mimic fed-batch /
        chemostat when cell density would otherwise strip the vessel.
    """
    ...
    if scale is not None and scale.get("enabled", False):
        P_ss = scale.get("P_ss")
        if P_ss is None:
            P_ss = steady_state_protein(gene)
        P_norm = P / (float(P_ss) + 1e-15)
        f_enz = scale["beta0"] + scale["beta1"] * P_norm / (1.0 + P_norm)
    ...
    if feed is not None and feed.get("enabled", False):
        dG_out += feed["k_feed"] * (feed["G_feed"] - G_out)
...
def default_initial_state(geo):
    """Match orders from glucose_to_heparin + exocytosis + bioreactor."""
    pathway0 = [
        0.0,
        0.0,
        0.0,
...
def run_standalone_diffusion_demo():
    """Forward-Euler loop for O₂ / uridine / adenosine (from diffusion.ipynb)."""
...
def main():
    ...
    def fun(t, y):
        return combined_synthetic_cell_ode(
            t, y, params, pulse_times, geo, exo, gene, scale, feed
        )
    ...
    ax = axes[1, 1]
    ax.plot(sol.t, sol.y[40], color="C0", label="mRNA")
    ax.set_xlabel("Time (min)")
    ax.set_ylabel("mRNA (arb.)", color="C0")
    ax.tick_params(axis="y", labelcolor="C0")
    ax.set_title("Gene expression (lumped)")
    ax.grid(True, alpha=0.3)
    ax3 = ax.twinx()
    ax3.plot(sol.t, sol.y[41], color="C1", label="Protein")
    ax3.set_ylabel("Protein (arb.; SS ≈ {:.0f})".format(steady_state_protein(gene)), color="C1")
    ...
    # Optional: short diffusion demo for other solutes (not coupled in ODE)
